...
export function ClipReveal({ children, className = "", delay = 0, style }: AnimProps) {
  ...
  const anim: CSSProperties = reduced
    ...
    : {
        ...
        opacity: inView ? 1 : 0,
        transition: `transform 0.85s var(--ease-out) ${delay}ms, opacity 0.65s var(--ease-out) ${delay}ms`,
...
export function SlideIn({
  ...
  const offset =
    from === "right" ? "translateX(28px)" : from === "left" ? "translateX(-28px)" : "translateY(24px)";
  const anim: CSSProperties = reduced
    ? {}
    : {
        opacity: inView ? 1 : 0,
        transform: inView ? "none" : offset,
        transition: `opacity 0.55s var(--ease-out) ${delay}ms, transform 0.65s var(--ease-out) ${delay}ms`,
        willChange: "transform, opacity",
      };
  return (
    <div ref={ref} className={className} style={{ ...anim, ...style }}>
  ...
  const anim: CSSProperties = reduced
    ...
    : {
        ...
        transform: inView
          ...
          : "perspective(1000px) rotateX(8deg) translateY(28px)",
        transformOrigin: "center bottom",
        transition: `opacity 0.5s var(--ease-out) ${delay}ms, transform 0.7s var(--ease-out) ${delay}ms`,
        willChange: "transform, opacity",
      };
  return (
    <div ref={ref} className={className} style={{ ...anim, ...style }}>
...
export function PopIn({ children, className = "", delay = 0, style }: AnimProps) {
  ...
  const anim: CSSProperties = reduced
    ...
    : {
        ...
        transition: `opacity 0.4s var(--ease-out) ${delay}ms, transform 0.45s var(--ease-out) ${delay}ms`,
        willChange: "transform, opacity",
      };
  return (
