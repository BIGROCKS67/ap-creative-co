          ...
          {items.map((item, i) => {
            ...
            return (
              <TiltCard key={item.title} delay={(i % 3) * 90} className="h-full">
                <div className="mm-card group h-full p-8">
                  <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-flow-green/10 transition-transform duration-200 [transition-timing-function:var(--ease-out)] [@media(hover:hover)_and_(pointer:fine)]:group-hover:scale-105">
